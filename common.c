 ...
 */
int** load_utilization(int *individual, int **H, int **V, int h_size, int v_size)
{
	/* iterators */
	...
	for (iterator_physical=0; iterator_physical < h_size; iterator_physical++)
	{
		...
		utilization[iterator_physical][0] = utilization[iterator_physical][1] = 
		utilization[iterator_physical][2] = 0;
	}
	...
	for (iterator_virtual = 0; iterator_virtual < v_size; iterator_virtual++)
	{
		...
		of Processor, Memory and Storage */
			requirements[individual[iterator_virtual]-1][0] += V[iterator_virtual][0];
			requirements[individual[iterator_virtual]-1][1] += V[iterator_virtual][1];
			requirements[individual[iterator_virtual]-1][2] += V[iterator_virtual][2];
		}
	}
	/* iterate on positions of an individual */
	for (iterator_physical=0; iterator_physical < h_size; iterator_physical++)
	{
		/* virtual machine requirements in Processor, Memory and Storage. Initialized to 0 */
		utilization[iterator_physical][0] = requirements[iterator_physical][0];
		utilization[iterator_physical][1] = requirements[iterator_physical][1];
		utilization[iterator_physical][2] = requirements[iterator_physical][2];
	}
	return utilization;
}
 ...
 */
float* load_objectives(int *individual, int **utilization, int **H, int **V, int **T, int h_size, int v_size, int *K, int *network_utilization, int l_size)
{
	/* iterators */
	...
	float *value_solution = (float *) malloc (5 *sizeof (float));
	/* iterate on individuals */
	power_consumption = 0.0;
	value_solution[0] = value_solution[1] = value_solution[2] = value_solution[3] = value_solution[4] = 0.0;
	...
	for (iterator_physical = 0 ; iterator_physical < h_size ; iterator_physical++)
	{
		if (utilization[iterator_physical][0] > 0)
		{
			...
			utilidad = (float)utilization[iterator_physical][0] / H[iterator_physical][0];
	...
	for (iterator_virtual = 0; iterator_virtual < v_size; iterator_virtual++) 
	{
		physical_position = individual[iterator_virtual];
		/* count the network traffic */
		count_traffic = 0;
		if (individual[iterator_virtual] != 0)
		{
			...
			{
				if (physical_position == individual[iterator_virtual2] || individual[iterator_virtual2] == 0) 
				{
					count_traffic = 0;
				} 
				else 
	...
	value_solution[1] = (float)network_traffic;
	...
	for (iterator_virtual = 0 ; iterator_virtual < v_size ; iterator_virtual++)
	{
		...
		if (physical_position > 0)
		{
			...
			value_solution[2] += (float) V[iterator_virtual][4];
			/* loads the QoS of each solution */
			value_solution[3] += (float) pow (CONSTANT,V[iterator_virtual][3]) * V[iterator_virtual][3];
		}
	}
	...
	{
		n_u = network_utilization[iterator_link] / 1.0f;
 ...
 * returns: utilization tridimentional matrix
 */
int*** load_population_utilization(int **population, int **H, int **V, int number_of_individuals, int h_size, int v_size)
{
	/* iterators */
	int iterator_individual;
	int iterator_virtual;
