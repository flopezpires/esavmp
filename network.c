/* 
 * network.c: Virtual Machine Placement Problem - Networking Functions
 * Date: 23-10-2015
...
int* load_network_utilization(int *individual, int **G, int **T, int l_size, int v_size)
{
	...
	int *network_utilization = (int *) malloc (l_size *sizeof (int));
	for (iterator_link=0; iterator_link < l_size; iterator_link++)
	{
		/* initialize link value */
		network_utilization[iterator_link] = 0;
	}
	...
	for (iterator_virtual=0; iterator_virtual < v_size; iterator_virtual++)
	{
		for (iterator_virtual_again=0; iterator_virtual_again < v_size; iterator_virtual_again++)
		{
			...
			if (individual[iterator_virtual] != individual[iterator_virtual_again] && individual[iterator_virtual] != 0 && individual[iterator_virtual_again] != 0)
			{
				/* for each network link */
				for (iterator_link=0; iterator_link < l_size; iterator_link++)
				{
					/* add the corresponding traffic for each used link */
					if (G[individual[iterator_virtual]-1][iterator_link] !=0 || G[individual[iterator_virtual_again]-1][iterator_link] !=0)
					{						
						network_utilization[iterator_link] += T[iterator_virtual][iterator_virtual_again];
					}
				}
			}
		}
