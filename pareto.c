...
void report_best_population(struct pareto_element *pareto_head, int **G, int **T, int **H, int **V, int v_size, int h_size, int l_size)
{
	...
	int ***utilization_best_P = load_population_utilization(best_P, H, V, pareto_size, h_size, v_size);
	int **network_utilization_best_P = load_population_network_utilization(best_P, G, T, pareto_size, l_size, v_size);
